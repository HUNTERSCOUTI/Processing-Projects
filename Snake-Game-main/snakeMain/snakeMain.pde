...
void setup(){
  ...
  highScore = new JSONObject();
  highScore = loadJSONObject("hs.json");
  oldBest = highScore.getInt("hs");
  ...
  pointC = new SoundFile(this, "pointC.wav");
...
void draw() {
  ...
  //In-Game Score
    ...
    //Outline maker - https://forum.processing.org/two/discussion/16700/how-to-outline-text
      ...
      if(oldBest > head.x.size()){
        highScore.setInt("hs", oldBest);
      } else {
        int newBest = head.x.size();
        highScore.setInt("hs", newBest);
      }
      saveJSONObject(highScore, "data/hs.json");
